...
class Errors:
    ...
    PASSWORD_TOO_SHOORT = Error('Password too short',
...
def setPassword():
    ...
    Parameters:
      String -- user: The target user's name.
      String -- token: The target user's token.
      String -- password: The target user's password in plaintext to set.
    Returns:
    ...
    targetUsers = User.query.filter_by(_name=user_requested).all()
    ...
    if targetUser.get_activation_token() != token:
        return (Errors.WRONG_TOKEN.to_json(),
    ...
    if len(password_requested) < 8:
        return Errors.PASSWORD_TOO_SHOORT.to_json(), status.HTTP_400_BAD_REQUEST
